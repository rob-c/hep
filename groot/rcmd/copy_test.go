...
func TestROOTCp(t *testing.T) {
	...
	cwd, err := os.Getwd()
	if err != nil {
		t.Fatalf("could not retrieve current work dir: %v", err)
	}
	defer os.Chdir(cwd)

	err = os.Chdir(dir)
	if err != nil {
		t.Fatalf("could not change to tmp dir %q: %v", dir, err)
	}
	...
	for _, tc := range []struct {
		...
		{
			name: "simple",
			ref:  filepath.Join(dir, "ref.root"),
		},
		{
			name: "relative",
			ref:  filepath.FromSlash("./ref.root"),
		},
		{
			name: "relative-sub",
			ref:  filepath.FromSlash("./sub1/ref.root"),
		},
		{
			name: "relative-sub-sub",
			ref:  filepath.FromSlash("./sub1/sub11/ref.root"),
		},
	} {
